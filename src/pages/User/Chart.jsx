    ...
    const fetchAllData = async () => {
      ...
      try {
        const [tempRes, humRes] = await Promise.all([
          axios.get(`http://localhost:8080/yolohome/adafruit/temp-chart/temp/${timeRange}`),
          axios.get(`http://localhost:8080/yolohome/adafruit/temp-chart/humidity/${timeRange}`),
        ]);
    ...
    <Box display="flex" justifyContent="center" mb={3}>
      <Card sx={{ width: "100%", maxWidth: 800, p: 2 }}>
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-end", p: 1 }}>
          <Button onClick={() => handleExportChart(ref, label)}>Export Chart</Button>
        </Box>
      </Card>
    </Box>
  );
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      {loading ? (
        ...
        <>
          {renderChart(chartDataTemp, "Biểu đồ nhiệt độ", "#1976d2", chartRefTemp)}
          {renderChart(chartDataHumidity, "Biểu đồ độ ẩm", "#388e3c", chartRefHumidity)}
        </>
      )}
    </Box>
  );
